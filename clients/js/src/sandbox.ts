...
type SandboxState = 'creating' | 'attaching' | 'running' | 'closed' | 'failed' | 'checkpointing' | 'checkpointed' | 'restoring' | 'filesystem_snapshotting' | 'reconnecting';
...
export class Sandbox {
  ...
  // private _authToken?: string; // Removed
  private _useGoogleAuth: boolean = false;
  private stdinBuffer: string[] = [];

  private static readonly HINT_AUTH_PERMISSION = " (Hint: Permission denied or missing authentication. Ensure you have enabled 'useGoogleAuth: true' and that your account has the 'Cloud Run Invoker' (roles/run.invoker) role on this service.)";
  ...
  private handleClose(code: number, reason: Buffer) {
    ...
    if (this.state === 'creating' || this.state === 'restoring' || this.state === 'attaching') {
      this.state = 'failed';
      const msg = Sandbox.appendErrorHint(`Connection closed: ${code} - ${reasonStr}`);
      const err = new Error(`Failed to connect to ${this._url}: ${msg}`);
  ...
  private handleError(err: Error) {
    ...
    if (this.state === 'creating' || this.state === 'restoring' || this.state === 'attaching') {
      this.state = 'failed';
      const finalError = new Error(`Failed to connect to ${this._url}: ${enhancedMsg}`);
      this.eventEmitter.emit('failed', finalError);
    }
  ...
  private async getReconnectInfo(): Promise<ReconnectInfo> {
    if (!this._sandboxId) {
